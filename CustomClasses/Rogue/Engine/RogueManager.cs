...
namespace Rogue.Engine
{
    public class RogueManager
    {
        ...
        readonly string[] Poisons = { "速效毒药", "速效毒药 I", "速效毒药 II", "速效毒药 III", "速效毒药 IV", "速效毒药 V", "速效毒药 VI" };
        ...
        public bool IsBuffRequired()
        {
            ...
            if (ObjectManager.Player.IsMainhandEnchanted() && ObjectManager.Player.IsOffhandEnchanted())
                return false;
            if (ObjectManager.Player.CastingId != 0 && ObjectManager.Player.CastingAsName != "潜行")
                return true;
            if (Inventory.ExistingItems(Poisons).Count > 0)
                return true;
        ...
        public bool IsReadyToFight(IEnumerable<WoWUnit> possibleTargets)
        {
            ...
            if (ObjectManager.Player.GotAura("进食"))
                return false;
        ...
        public void Pull(WoWUnit target)
        {
            ...
            Stealth();
            if (ObjectManager.Target.DistanceToPlayer >= 20 && Spell.IsKnown("疾跑") && Spell.IsReady("疾跑"))
                Spell.Cast("疾跑");
            if (Spell.IsKnown("偷袭") && Spell.IsReady("偷袭"))
                Spell.Cast("偷袭");
            else
                Spell.Attack();
        ...
        void Rotation()
        {
            ...
            int ComboPoint = ObjectManager.Player.ComboPoints;
            Spell.Attack();
            if (Spell.IsKnown("狂暴") && Spell.IsReady("狂暴"))
                Spell.Cast("狂暴");
            if (Spell.IsKnown("血性狂怒") && Spell.IsReady("血性狂怒"))
                Spell.Cast("血性狂怒");
            if (PossibleTargets.Where(x => x.TargetGuid == ObjectManager.Player.Guid).Count() >= 2)
            {
                if (Spell.IsKnown("冲动") && Spell.IsReady("冲动"))
                    Spell.Cast("冲动");
                if (Spell.IsKnown("血性狂怒") && Spell.IsReady("血性狂怒"))
                    Spell.Cast("血性狂怒");
                if (Spell.IsKnown("剑刃乱舞") && Spell.IsReady("剑刃乱舞") && Energy >= 25)
                    Spell.Cast("剑刃乱舞");
                if (Spell.IsKnown("闪避") && Spell.IsReady("闪避"))
                    Spell.Cast("闪避");
            }
            if (Spell.IsKnown("还击") && Spell.IsReady("还击") && Energy >= 10)
                Spell.Cast("还击");
            if (Energy <= 20)
                return;
            if (Energy >= 25 && (ObjectManager.Target.CastingId != 0 || ObjectManager.Target.ChannelingId != 0))
            {
                if (Spell.IsKnown("脚踢") && Spell.IsReady("脚踢"))
                    Spell.Cast("脚踢");
            }
            if (Energy >= 35)
            {
                if (Spell.IsKnown("刺骨") && Spell.IsReady("刺骨"))
                {
                    if (ShouldEviscerate() || ComboPoint == 5)
                    {
                        Spell.Cast("刺骨");
            ...
            if (Energy >= 25)
            {
                if (Spell.IsKnown("切割"))
                {
                    if ((!ObjectManager.Player.GotAura("切割") || GetSliceAndDiceDuration() <= 2.0) && !ShouldEviscerate() && ComboPoint > 0)
                    {
                        Spell.Cast("切割");
                        return;
                    }
        ...
        bool ShouldEviscerate()
        {
                int rank = Spell.GetSpellRank("刺骨");
                int damage = 0;
                int comboPoints = ObjectManager.Player.ComboPoints;
                if (rank >= 1 && comboPoints >= 1)
                {
        ...
        void Stealth()
        {
            if (Spell.IsKnown("潜行"))
            {
                if (!ObjectManager.Player.GotAura("潜行") && Spell.IsReady("潜行") && ObjectManager.Target.DistanceToPlayer < 25f)
                    Spell.CastWait("潜行", 500);
            }
        }
